...
Community Notes／Agora 那一類 bridging-based ranking 的單因子版：`r_ij ≈ μ + a_i + b_j + f_i·g_j`（agree=1、disagree=-1、pass=0，未投不計），交替最小平方加 L2 正則化（截距 0.03、因子 0.15）跑 40 輪，亂數種子由對話 id 與票數決定，結果可重現。`b_j` 是扣掉立場軸之後的同意程度（橋接分數），`|g_j|` 正規化後是極化度。同樣的原始同意率，被單一陣營撐起來的陳述分數較低、極化度較高。只輸出陳述層級的 `score`、`polarity`、票數，不輸出參與者的 `a`、`f`；納入分群者不足 4 人或陳述票數低於 max(3, 10%) 者不列。與第 5 節的 group-aware consensus 互補：那一個看「每一群都傾向同一邊」，這一個不依賴分群結果。
